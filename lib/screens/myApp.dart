        ...
        actions: <Widget>[
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              var collectionPeding = FirebaseFirestore.instance
                  .collection(ls[0] + ls[1])
                  .doc('pendingApproval')
                  .collection('pending');
              //.doc(mainDb)
              //.collection(col + each);
              var snapshotsPending = await collectionPeding.get();
              for (var doc in snapshotsPending.docs) {
                await doc.reference.delete();
              }

              var collectionFormula = FirebaseFirestore.instance
                  .collection(ls[0] + ls[1])
                  .doc(mainDb)
                  .collection('formula');
              var snapshotsFormula = await collectionFormula.get();
              for (var doc in snapshotsFormula.docs) {
                await doc.reference.delete();
              }
              ...
              var collection =
                  FirebaseFirestore.instance.collection(ls[0] + ls[1]);
              //.doc(mainDb)
              //.collection(col + each);
              var snapshots = await collection.get();
              for (var doc in snapshots.docs) {
              ...
              //.doc(mainDb)
              //.collection(col + each);
              var snapshotsUsers = await collectionUsers.get();
              for (var doc in snapshotsUsers.docs) {
                await doc.reference.delete();
              }
